...
export async function GET(request: Request) {
  ...
  // Grab the actual user's IP and Device Type from the incoming request headers
  const userIp = request.headers.get("x-forwarded-for") || ""
  const userAgent = request.headers.get("user-agent") || ""

  // Private AdBlueMedia API URL (Appending the user IP and User Agent dynamically)
  ...
  try {
    ...
    const mappedOffers = rawOffers.map((offer: any, index: number) => ({
      ...
      // Use 'title' for the real name, fallback to 'name' or 'anchor' if title is missing
      name: offer.title || offer.name || offer.anchor || "Complete Offer",
      // Use 'description' if they provide one, fallback to 'conversion' instructions
      description: offer.description || offer.conversion || "Follow the instructions to complete.",
